...
const mainJsStr = fs.readFileSync('main.js', 'utf8');

for (const [name, id] of Object.entries(manifest)) {
    // Find the relative path in main.js
    const regex = new RegExp(`"([^"]*?${name.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')})"`, 'g');
    const match = regex.exec(mainJsStr);
    
    if (match) {
        const relativeDest = match[1];
        
        // Ensure folder exists
        const folder = path.dirname(relativeDest);
        ...
        if (fs.existsSync(relativeDest)) {
            const stats = fs.statSync(relativeDest);
            if (stats.size > 1000) {  // If it's more than 1KB, it's not a pointer
                console.log(`[SKIP] Already downloaded: ${relativeDest}`);
                downloadNeeded = false;
            }
        }
        
        if (downloadNeeded) {
            console.log(`\n[DOWNLOADING] Overwriting pointer: ${relativeDest}`);
            try {
                // Using gdown to bypass virus scan warnings explicitly
                execSync(`python -m gdown --id "${id}" -O "${relativeDest}"`);
                console.log(`[SUCCESS] -> ${relativeDest}`);
            } catch (err) {
