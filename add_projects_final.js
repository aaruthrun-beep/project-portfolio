...
// We replace the last ]; with our new projects and a final ];
// Since there might be multiple ];, we find the one after smart rover.
const smartRoverIdx = code.indexOf('smart rover');
const lastBracketIdx = code.indexOf('];', smartRoverIdx);

if (lastBracketIdx !== -1) {
    code = code.substring(0, lastBracketIdx) + newContent + code.substring(lastBracketIdx + 2);
    fs.writeFileSync('main.js', code);
    console.log('Successfully added missing projects to main.js');
} else {
    console.error('Could not find the end of projects array');
}
