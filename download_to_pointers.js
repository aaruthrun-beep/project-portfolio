...
function walkDir(dir) {
    ...
    list.forEach((file) => {
        ...
        if (stat && stat.isDirectory()) {
            if (!file.includes('node_modules') && !file.includes('.git') && !file.includes('ALL_VIDEOS')) {
...
const allMp4s = walkDir(__dirname);
...
for (const filepath of allMp4s) {
    ...
    if (id) {
        ...
        if (stats.size < 1000) {
            console.log(`\n[DOWNLOADING] Overwriting LFS pointer: ${filepath}`);
            try {
                // Using Python's gdown to securely download Google Drive blobs bypassing the virus scan
                execSync(`python -m gdown --id "${id}" -O "${filepath}"`);
                console.log(`[SUCCESS] -> ${filepath}`);
            } catch (err) {
                console.error(`[ERROR] Failed to download ${filename}`);
            }
        } else {
            console.log(`[SKIP] Already a real video file: ${filepath} (${stats.size} bytes)`);
        }
    }
}
