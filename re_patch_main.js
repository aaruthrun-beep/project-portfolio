...
// The 3 missing projects with professional descriptions
const newProjects = `
    {
        id: 21,
...
// We find the last category section (SCHOOL) and add it there.
// Or just find the end of the array.
if (code.includes('smart rover') && !code.includes('veel koothu')) {
    code = code.replace(/}\s*\];/, `},${newProjects}\n];`);
    console.log('Added 3 missing projects.');
}

// Fix the thumbnail encoding issue by using double quotes in style
code = code.replace(/background-image:url\('\$\{thumbUrl\}'\)/g, 'background-image:url("${thumbUrl}")');

fs.writeFileSync(path, code);
console.log('Main.js updated with new projects and thumbnail fix.');
